...
SELECT  DATE_TRUNC(estg.event_date_nk, MONTH)             event_date_nk,
        ...
        estg.reduced_page_location                        page_location
        COALESCE(LOWER(molg.page_location), 'untracked')  page_location_tracked,
  ...
  FROM  event_stg estg
  LEFT  JOIN gcp_gs.map_{business_type}_location_groups molg
